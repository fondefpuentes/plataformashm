...
def getThreshold(puente_id, nombre_sensor):
    umbrales = []
    for eje in range(3):
        df = getCoef(puente_id, nombre_sensor, eje)
        gb = df.groupby('reporte_dano_id')
        ...
        coefdf = pd.DataFrame(coef_sensor)
        mh = mahalanobis(x=coefdf, data=coefdf)
        umbrales.append(analizeTh(mh, puente_id))
    return umbrales

def analizeTh(mahal, id_puente):
    config = ConfiguracionModeloAR.query.filter_by(id_estructura = id_puente).first()
    count = 0
    for dist in mahal:
        if dist > config.umbral_distancia:
...
def addAnomallyAll(id_puente, sensores):
    try:
        for sensor in sensores:
            ...
            umbrales = getThreshold(id_puente, sensor.nombre_sensor)
            ...
            if(anomalia.anomalia != damage):
                anomalia = AnomaliaPorHora(id_sensor_instalado = sensor.si, hora_calculo = ahora, anomalia = damage, umbralx = umbrales[0], umbraly = umbrales[1], umbralz = umbrales[2])
...
def propagation():
    ...
    _estados_estructura = ['Sin Daño', 'Alerta general instantánea', 'Alerta general intermitente', 'Daño general']
    CLT = pytz.timezone('America/Santiago')
